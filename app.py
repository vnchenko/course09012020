...
app.debug = False
app.config['SQLALCHEMY_DATABASE_URI'] = 'postgres://xodvmwtdeedsnp:8a6889d02b6c64ec8662ca4d7fb085a79b3703d3aa01899af03d30768f708f65@ec2-107-22-216-123.compute-1.amazonaws.com:5432/dbbm43jhp6ole5'
    ...
    if request.method == 'POST':
        if form.validate():
            try:
                ...
                form.user_email.errors = ['this user is registered']
    ...
    if request.method == 'POST':
        ...
        if form.validate() and form.validate_date():
            try:
                ...
                form.user_email.errors = ['this user already exists']
...
def edit_skill(email, skill):
    ...
    if request.method == 'GET':
        ...
        if form.validate():
            try:
                result.skill_name = form.skill_name.data.strip()
                db.session.commit()
    ...
    if request.method == 'POST':
        if form.validate():
            try:
                ...
                form.criterion_name.errors = ['this cruterion already exists']
